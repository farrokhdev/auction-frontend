...
function Search() {
  ...
  const search = () => {
    setLoading(true);
    axios
      .get(`${BASE_URL}/sale/search/?${queries}`)
      .then((resp) => {
        if (resp.status === 200) {
          setItems(resp.data);
          setLoading(false);
          ...
          >
            ...
            <div className="block-body">
              <h3 className="default">
                {item?.home_auction_name ? item?.home_auction_name : "---"}
              </h3>
              <h6 className="default">
                {item?.home_auction_type ? item?.home_auction_type : "---"}
              </h6>
            </div>
          </Link>
          //     <div className="row">
  ...
  const SearchResults = (data) => {
    ...
    if (data.data.hasOwnProperty("auctions")) {
      return data.data.auctions.map((item, key) => {
        return SearchType("auctions", item);
      });
    }
    if (data.data.hasOwnProperty("products")) {
      return data.data.products.map((item, key) => {
        return SearchType("products", item);
      });
    }
            ...
            <div className="row">
              <div className="col-lg-6">
                <div className="main-search">
                  <input
                    type="text"
                    placeholder="جستجوی اثر، حراج‌، خانه حراج"
