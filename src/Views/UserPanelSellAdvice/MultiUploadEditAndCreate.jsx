      ...
      return [
        ...
        <div className="default-checkbox-image-upload d-flex justify-content-end ">
          <input
            ...
            type="radio"
          />
          <span className="pb-2 mx-2">پیشفرض</span>
        </div>,
      ];
    },
  ...
  return (
    <React.Fragment>
              ...
              if (res.data.data.result.upload_url && (file?.type.split("/")[0] === "image")) {
                ...
                UploadAxios.put(res.data.data.result.upload_url , file)
                  .then((res) => {
                    ...
                    message.success(`بارگذاری تصویر با موفقیت انجام شد`);
                  })
                  .catch((err) => {
                    console.error(err);
                    onError({ status: "error" });
                    message.error(`بارگذاری تصویر با خطا مواجه شد`);
                  });
      ...
      >
        ...
        <p className="ant-upload-text">تصاویر خود را اینجا رها کنید</p>
      </Dragger>
    </React.Fragment>
  );
}
